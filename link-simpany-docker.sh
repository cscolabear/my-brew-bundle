...
target=".env"
source_file="$SOURCE_ROOT_PATH/simpany/Docker/simpany-docker-dev/$target"
target_file="$LINK_DOCKER_PROJECT_SIMPANY_PATH/simpany-docker-dev/$target"
...
target=".env"
source_file="$SOURCE_ROOT_PATH/simpany/Docker/docker-clamAV/$target"
target_file="$LINK_DOCKER_PROJECT_SIMPANY_PATH/docker-clamAV/$target"
echo "🧪 try: ln -s ${source_file} ${target_file}"
